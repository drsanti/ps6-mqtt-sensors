 ...
 *
 * Description 	: FreeRTOS task that subscribes to MQTT_SUB_TOPIC and actuates
 *                the user LED based on MQTT subscriber callback notifications.
 *
    ...
    while (true)
    {
        ...
        if (pdTRUE == xQueueReceive(subscriber_task_q, &subscriber_q_data, portMAX_DELAY))
        {
            switch(subscriber_q_data.cmd)
            {
                ...
                {
                    /* Update the current device state (LED control removed). */
