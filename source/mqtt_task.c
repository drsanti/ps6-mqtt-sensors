 ...
 *
 * Description 	: FreeRTOS task for Wi-Fi and MQTT client init/connect; starts
 *                subscriber and publisher tasks; reconnection and cleanup.
 *
...
/*String that describes the MQTT handle that is being created in order to
...
/* Set when MQTT is connected; cleared on disconnect/cleanup. Sensor task uses
 * this to publish only when connected. */
...
static void print_mqtt_broker_info(void);
...
static cy_rslt_t
mqtt_get_unique_client_identifier(char *mqtt_client_identifier);
...
void mqtt_client_task(void *pvParameters) {
  ...
  if (pdPASS != xTaskCreate(subscriber_task, "Subscriber task",
  ...
  if (pdPASS != xTaskCreate(publisher_task, "Publisher task",
...
/******************************************************************************
 * Function Name: mqtt_is_connected
 ******************************************************************************/
int mqtt_is_connected(void) { return mqtt_connected; }
...
static cy_rslt_t wifi_connect(void) {
  ...
  if (cy_wcm_is_connected_to_ap() == 0) {
    ...
    for (uint32_t retry_count = 0; retry_count < MAX_WIFI_CONN_RETRIES;
         retry_count++) {
      ...
      printf("Wi-Fi Connection failed. Error code:0x%0X. Retrying in %d ms. "
             "Retries left: %d\n",
...
static cy_rslt_t mqtt_init(void) {
  ...
  mqtt_network_buffer =
      (uint8_t *)pvPortMalloc(sizeof(uint8_t) * MQTT_NETWORK_BUFFER_SIZE);
  ...
  if (CY_RSLT_SUCCESS == result) {
    ...
    result = cy_mqtt_register_event_callback(
        mqtt_connection, (cy_mqtt_callback_t)mqtt_event_callback, NULL);
    if (CY_RSLT_SUCCESS == result) {
...
static cy_rslt_t mqtt_connect(void) {
  ...
  CHECK_RESULT(
      result, 0,
      "Failed to generate unique client identifier for the MQTT client!\n");
#endif /* GENERATE_UNIQUE_CLIENT_ID */
  ...
  printf("\n'%.*s' connecting to MQTT broker '%.*s'...\n",
         connection_info.client_id_len, connection_info.client_id,
         broker_info.hostname_len, broker_info.hostname);

  for (uint32_t retry_count = 0; retry_count < MAX_MQTT_CONN_RETRIES;
       retry_count++) {
    if (cy_wcm_is_connected_to_ap() == 0) {
...
static void print_mqtt_broker_info(void) {
  printf("MQTT broker: %.*s:%d (%s)\r\n", broker_info.hostname_len,
         broker_info.hostname, (int)broker_info.port,
  ...
  printf("MQTT session: clean=%d keepalive=%lu sec\r\n",
         (int)connection_info.clean_session,
         (unsigned long)connection_info.keep_alive_sec);
  printf("MQTT topics: pub='%s' sub='%s' qos=%d\r\n", MQTT_PUB_TOPIC,
         MQTT_SUB_TOPIC, (int)MQTT_MESSAGES_QOS);
}
